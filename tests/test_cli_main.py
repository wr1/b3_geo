...
from b3_geo.cli import main


def test_cli_main_default_to_loft(tmp_path):
    """Test CLI main defaults to loft when one arg not a command."""
    config_data = {
        "general": {"workdir": "."},
        "geometry": {
            "planform": {
    ...
    with patch("sys.argv", ["b3-geo", str(config_file)]):
        main()
