...
def _ctrl_pts(value):
    """Extract list of (x, y) pairs from a planform field value (plain list or typed dict)."""
...
def plot_planform(
    ...
    for i, (name, values) in enumerate(params.items()):
        ...
        if raw:
            pts = _ctrl_pts(raw)
            if pts:
                ctrl_xs, ctrl_ys = zip(*sorted(pts))
            ...
            chord_points = sorted(_ctrl_pts(controls["chord"]))
            thick_points = sorted(_ctrl_pts(controls["thickness"]))
            if not chord_points or not thick_points:
                continue
