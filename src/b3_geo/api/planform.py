...
from b3_geo.utils.interpolation import eval_curve
...
def interpolate_planform(planform_data, npspan):
    ...
    interp_plan = {
        "rel_span": rel_span,
        "z": eval_curve(planform_data["z"], rel_span, default_method="linear"),
        "chord": eval_curve(planform_data["chord"], rel_span),
        "thickness": eval_curve(
            planform_data["thickness"], rel_span, default_method="cubic_natural"
        ),
        "twist": eval_curve(planform_data["twist"], rel_span),
        "dx": eval_curve(planform_data["dx"], rel_span, default_method="cubic_natural"),
        "dy": eval_curve(planform_data["dy"], rel_span),
    }
